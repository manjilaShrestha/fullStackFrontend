...
const EyeSlashIcon = (
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const res =  await privateAPI.get("/employees", config);
  ...
  const fetchDepartments = async () => {
    try {
      const res = await privateAPI.get("/departments");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await privateAPI.put(`/employees/${editingId}`, formData);
        // await axios.put(
        //   `http://localhost:3152/api/employees/${editingId}`,
        //   formData
        // );
      } else {
        const res = await privateAPI.post("/employees/create", formData);
      }
      toast.success(
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await privateAPI.delete(`/employees/${id}`);
